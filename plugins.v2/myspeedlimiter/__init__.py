...
class mySpeedLimiter(_PluginBase):
    ...
    plugin_version = "2.0"
    # 插件作者
    plugin_author = "ting1e"
    # 作者主页
    author_url = "https://github.com/ting1e"
    # 插件配置项ID前缀
    plugin_config_prefix = "myspeedlimit_"
    # 加载顺序
    plugin_order = 11
    # 可使用的用户级别
    auth_level = 1
    ...
    def check_playing_sessions(self, event: Event = None):
        ...
        if total_bit_rate:
            ...
            self._unlimited_current = self._unlimited_interval
        else:
            if self._unlimited_current > 0 :
                self._unlimited_current -= self._interval
            else:
            # 当前没有播放，取消限速
